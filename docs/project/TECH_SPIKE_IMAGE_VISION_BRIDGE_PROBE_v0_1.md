...
**IAM note:** Service account needs **`roles/aiplatform.user`** (or broader) on `hearing-aid-mvp` for Vertex `generateContent`. Same SA already has `discoveryengine.user` for `:answer` — verify Vertex role before first run.
...
| `IMAGE_VISION_PROBE_LOCATION` | No | Falls back to `AGENT_SEARCH_LOCATION` → `CES_LOCATION` (use `eu`) |
